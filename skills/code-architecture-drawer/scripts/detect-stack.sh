#!/usr/bin/env bash
# Detect the project's languages and tooling before mapping its architecture.
...
printf '\nRead the framework'\''s conventions before naming layers: a Laravel app/Models or a Rails app/models folder is persistence, not a pure domain.\n'
